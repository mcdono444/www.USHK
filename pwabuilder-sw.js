...
self.addEventListener('fetch', (event) => {
  if (event.request.mode === 'navigate') {
    ...
    // ⚠️ Só deixa passar direto (sem SW) requisições de LOGIN/REGISTRO
    // do PRÓPRIO site e de domínios externos de autenticação/Firebase.
    // IMPORTANTE: não use "onrender.com" aqui — seu site inteiro está
    // nesse domínio, e isso desativaria o cache de TODAS as páginas.
    const isSameOrigin = url.origin === self.location.origin;
    const isAuthPage = isSameOrigin && (
      url.pathname.includes('/login.html') ||
      url.pathname.includes('/register.html') ||
      url.pathname.includes('/auth')
    );
    const isExternalAuthDomain =
      !isSameOrigin && (
    ...
    if (isAuthPage || isExternalAuthDomain) {
...
self.addEventListener('fetch', (event) => {
  ...
  if (event.request.mode === 'navigate') return; // já tratado acima
